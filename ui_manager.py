...
class UIManager:
    ...
    def update_ui(self, text, is_final, source="system"):
        self.root.after(0, self._render_text, text, is_final, source)
   
    def _render_text(self, text, is_final, source="system"):
        ...
        if is_final:
            ...
            tag = "system_final" if source == "system" else "mic_final"
            prefix = "SYS: " if source == "system" else "YOU: "
            ...
            tag = "system_live" if source == "system" else "mic_live"
            prefix = "SYS >> " if source == "system" else "YOU >> "
            self.text_area.insert(tk.END, f"{prefix}{text}", tag)
            self.text_area.see(tk.END)
